...
client.on('message', message => {
  ...
  if (msg.startsWith('?') || msg.startsWith('where') || msg.startsWith('what') || msg.startsWith('how') || msg.startsWith('when') || msg.startsWith('why')) {
	  if (!doFun) return;
	  
	  message.channel.send('https://www.google.com/search?q=' + message.content.replace(/ /g, '+').replace(/@/g, ''));
  }
  
  if (!message.member.hasPermission('ADMINISTRATOR')) return;
  
  if (msg === '!toggle-fun') {
	  let json = JSON.parse(filesystem.readFileSync('config.json', 'utf-8'));
	  json['doFun'] = !json['doFun'];
	  
	  filesystem.writeFileSync('config.json', JSON.stringify(json, null, '\t'));
	  doFun = json['doFun'];
	  
	  message.channel.send('Fun: ' + doFun);
  }
...
class Bot {
  ...
  startBot = () => {
	...
	this.mcBot.on('kicked', (reason, loggedIn) => {
