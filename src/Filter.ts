export class Filter {
	public static complies(words: string[], element: string): boolean {
	...
	private static testWithPhrase(test: string, word: string): boolean {
		let words: string[] = word.split("|");
		for (let i = 0; i < words.length; i++) { if (!test.includes(words[i])) { return false; } }
		return true;
	}

	private static testWithWord(test: string, word: string): boolean {
